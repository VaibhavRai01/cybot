...
if __name__ == "__main__":
    ...
    class ButtonYesNoduel_end(View):
        ...
        async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            if str(interaction.user.id) == str(self.user1):
                ...
                await interaction.edit_original_response('Tere lie nahi tha bhai', ephemeral=True)
    ...
    def updateProblemDatabase():
        ...
        for x in problem:
            ...
            if x.get('tags') is None:
                tags = ['NULL']
            else:
                tags = x['tags']
                tag_bool = [0 for i in range(40)]
                ...
                if 'brute force' in tags:
                    tag_bool[5] = 1
                if 'constructive algorithms' in tags:
                    tag_bool[6] = 1
                if 'graphs' in tags:
                    tag_bool[7] = 1
                if 'sortings' in tags:
                ...
                if 'dsu' in tags:
                    tag_bool[18] = 1
                if 'shortest paths' in tags:
                    tag_bool[19] = 1
                if 'probabilities' in tags:
                    tag_bool[20] = 1
                if 'divide and conquer' in tags:
                    tag_bool[21] = 1
                if 'hashing' in tags:
                    tag_bool[22] = 1
                if 'games' in tags:
                ...
                if 'matrices' in tags:
                    tag_bool[26] = 1
                if 'string suffix structures' in tags:
                    tag_bool[27] = 1
                if 'fft' in tags:
                    tag_bool[28] = 1
                if 'graph matchings' in tags:
                    tag_bool[29] = 1
                if 'ternary search' in tags:
                    tag_bool[30] = 1
                if 'expression parsing' in tags:
                    tag_bool[31] = 1
                if 'meet-in-the-middle' in tags:
                    tag_bool[32] = 1
                if '2-sat' in tags:
                    tag_bool[33] = 1
                if 'chinese remainder theorem' in tags:
                    tag_bool[34] = 1
                if 'schedules' in tags:
                    tag_bool[35] = 1
                json_tag = '{' + ','.join(str(x) for x in tag_bool) + '}'
            cur.execute('INSERT INTO PROBLEMS VALUES(%s,%s,%s,%s,%s)', (contestId, index, rating, name, json_tag,))
            conn.commit()
    ...
    async def reminder():
        ...
        channel = bot.get_channel(1040364945201696829)
        response = requests.get(url)
        data = response.json()
        result = data['result']
    ...
    async def solo_arise(Interaction: discord.Interaction, rating: int = None, tags: str = None):
        ...
        if roww:
            ...
            conn.commit()
            rows = cur.fetchall()
            linktobesold = "https://codeforces.com/contest/" + str(rows[0][2]) + "/problem/" + str(rows[0][3])
            desc = f"You are already in solo ([problem]({linktobesold})). End your ongoing solo by using solo_end command"
            ti = 'Already in solo'
            embed = send_embed(ti, desc)
            await Interaction.edit_original_response(embed=embed)
            return
        ...
        if row is None:
            ...
            if rating is not None and (rating < 800 or rating > 3500 or rating % 100 != 0):
                ...
                if not unsolvedprobs:
                    desc = 'Nahi hai bhai question Bank me'
                    ti = 'No problem found'
                    embed = send_embed(ti, desc)
                    await Interaction.edit_original_response(embed=embed)
                    return
                unsolgiven = random.choice(unsolvedprobs)
                while unsolgiven[0] < 1599:
                    unsolgiven = random.choice(unsolvedprobs)
                now = datetime.now()
                now1 = str(now)
                now1 = now1.split(".")
    ...
    async def daily_problem(Interaction: discord.Interaction, contestid: str, contestindex: str):
        ...
        if Interaction.channel_id != 1056929299501953104:
            await Interaction.edit_original_response(
                content="This command cannot be used here OR you are not the chosen one ")
            return
